...
To assign network traffic to users different attributes were sent to zabbix like username, ip and port on requests to a modified datanode.
Combined with the information from software defined network it was possible to get an overview of current user bandwidth. Further the traffic amount for a time range was calculated. The namenode was used for sending the current user storage amount to the monitoring system.

The namenode is responsible to establish the first client connection. By sending a file download request it will respond with a list of file blocks, and the assigned datanode location. By varying the block list, different power consumption optimizations were analyzed.
